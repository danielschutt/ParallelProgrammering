    ...
    public static void main(String[] argv) {
        try {
            ...
            for (int run = 0; run < runs; run++) {
                ...
                for(int i=0; i<ntasks;i++) {
                	Future<List<Integer>> myFuture = futures.get(i);
                	List<Integer> individualResults = myFuture.get();
                	//System.out.println( "\n\n Amount of results in task:" + individualResults.size());
                	for(int j=0; j<individualResults.size();j++) {
                		result.add(individualResults.get(j));
                	}
